...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None and attention_mask.dim() == 2:
            ...
            )  # Mask is 0 @ real channels and 1 @ padded channels.
            attention = attention.masked_fill(attention_mask, min_value)
            ...
            raise ValueError("Attention Mask should have dim == 2 or dim == 4")
        ...
        *dims, n_heads, seq, hid = weighted.size()
        ...
        # count up the flops, for a single batch element
        self.maccs = (
            queries.shape[-1] * queries.shape[-2] * keys.shape[-2] * self.num_heads
        )
        self.maccs += (
            attention.shape[-1]
            * attention.shape[-2]
        ...
        self.maccs += (
            self.num_heads * attention.shape[-2] * attention.shape[-1] * 3
